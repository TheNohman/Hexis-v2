  ...
  await prisma.$transaction(async (tx) => {
    for (const block of workout.blocks) {
      for (const entry of block.entries) {
        // Copy planned values to actual values for each KPI value row.
            ...
            create: b.entries.map((e) => ({
              ...
              values: {
                create: e.values.map((v) => {
                  // Seed planned values from the previous actual values.
                  // Apply weight bump only on weight KPIs for STRENGTH
                  // exercises.
