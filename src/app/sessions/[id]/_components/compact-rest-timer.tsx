...
export function CompactRestTimer({ durationSecs, onComplete }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between px-5 h-14 bg-surface shadow-hero border-t border-border">
        <span className="text-[10px] uppercase tracking-widest font-semibold text-muted">
          Repos
        </span>
        <span className="font-display font-black text-2xl tabular-nums text-accent-ink">
          {formatDuration(remaining)}
        </span>
        <button
          type="button"
          onClick={skip}
          className="min-h-[44px] min-w-[44px] flex items-center justify-center text-sm font-medium text-muted hover:text-foreground cursor-pointer transition-colors"
        >
          Passer
        </button>
      </div>
