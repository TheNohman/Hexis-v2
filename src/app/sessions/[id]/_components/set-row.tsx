...
import { skipEntryAction, updateEntryValuesAction, validateEntryAction } from "@/app/sessions/actions";
...
export function SetRow({ workoutId, entry, setNumber, onValidated }: Props) {
  ...
  return (
    <div className={`flex items-center gap-2 px-4 py-2.5 transition-colors ${
      isDone ? "bg-done-light" : isSkipped ? "bg-surface opacity-50" : isPlanned ? "bg-planned-light" : ""
    }`}>
      <span className="text-xs text-muted w-6 text-center shrink-0 tabular-nums font-medium">{setNumber}</span>
